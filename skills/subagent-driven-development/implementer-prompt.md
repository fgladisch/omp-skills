...
This file's content is the **`task` string** for an `agent: "worker"` dispatch. The controller fills in the `[bracketed]` placeholders, then passes the entire content as the `task` field to `subagent({...})`.
...
**Ask them now.** Raise any concerns before starting work. Use the standard agent reply mechanism — do not call `intercom` unless an explicit bridge target was provided in this prompt.
...
**While you work:** If you encounter something unexpected or unclear, **ask questions**. It is always OK to pause and clarify. Don't guess or make assumptions.
