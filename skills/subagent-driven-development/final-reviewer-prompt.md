...
Read the full plan at `PLAN_PATH`, inspect the current files at `HEAD_SHA`, and use the exact base-to-head range above. Assess the supplied verification evidence. Run only targeted checks when that evidence is missing, stale, or contradicted by the implementation; do not rerun the full verification suite.
